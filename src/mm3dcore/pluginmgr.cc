...
bool PluginManager::loadPluginDir(const char *pluginDir)
{
	if(pluginDir&&pluginDir[0])
	{
		...
		do {
			...
			if(WideCharToMultiByte(CP_UTF8,WC_ERR_INVALID_CHARS,ffd.cFileName,-1,&file[0],utf8Size,nullptr,nullptr)==0)
			{
		...
		if(dp)
		{
			...
			while((d = readdir(dp))!=nullptr)
			{
				...
				std::string file = pluginDir;
				file.push_back(DIR_SLASH);
				file += d->d_name;
...
int_list PluginManager::getPluginIds()
{
	int_list pluginList;
...
std::string PluginManager::fileToName(const char *file)
{
...
void PluginManager::disable(const char *pluginName)
{
	...
	{
		if(PORT_strcasecmp ((*it).c_str(),pluginName )==0)
		{
			return;
		}
	}
