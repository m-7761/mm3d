...
class DataDest
{
	public:
		...
		// implementing a DataDest.
		virtual /*ssize_t*/intptr_t writeVPrintf(const char *format,va_list ap);

		// Writes a null-terminated string to output (including null)
		/*ssize_t*/intptr_t writeAsciiz(const char *str);

		// Writes a null-terminated string to output (not including null)
		/*ssize_t*/intptr_t writeString(const char *str);
