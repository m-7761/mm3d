...
static Md3Filter *s_filter = nullptr;
#endif // PLUGIN
...
{
	std::string animFile = m_modelPath+"animation.cfg";
	int animCount = 0;
	...
	else
	{
			...
			else
			{
				...
				if(sscanf(line,"%d %d %d %d %s %s",
					...
					||(sscanf(line,"%s %d %d %d %d",
						 ...
						 &&(PORT_strncasecmp(fname,"all_",4)==0||PORT_strncasecmp(fname,"both_",5)==0
							||PORT_strncasecmp(fname,"torso_",6)==0||PORT_strncasecmp(fname,"legs_",5)==0
						 ...
						 &&(animKeyword||(animKeyword = 1))))
				{
					...
					{
						// "//ALL_x..."
						if(strlen(comment)>6&&PORT_strncasecmp(comment,"//",2)==0)
						{
							name = &comment[2];
						}
						// "ALL_x..."
					...
					if(PORT_strncasecmp(name,"all_",4)==0)
					{
						// Check for single frame dead animation that use the last
						//	frame of the death animation
						if(animCount>0&&PORT_strncasecmp(name,"all_dead",8)==0
						  &&PORT_strncasecmp(animNames[animCount-1].c_str(),"all_death",9)==0
						  &&fcount<=1)
						{
					...
					else if(PORT_strncasecmp(name,"both_",5)==0)
					{
						...
						// Check for single frame dead animation that use the last
						//	frame of the death animation
						if(animCount>0&&PORT_strncasecmp(name,"both_dead",9)==0
						  &&PORT_strncasecmp(animNames[animCount-1].c_str(),"both_death",10)==0
						  &&fcount<=1)
						{
							if(m_animStartFrame[animCount-1]+last_fcount-1==first)
							{
					...
					else if(PORT_strncasecmp(name,"torso_",6)==0)
					{
						...
						if(PORT_strcasecmp(name,"torso_stand")==0)
						{
							m_standFrame = first;
						}
					}
					else if(PORT_strncasecmp(name,"legs_",5)==0)
					{
						if(legsStart==-1)
						{
							legsStart = first;
						...
						if(PORT_strcasecmp(name,"legs_idle")==0)
						{
							m_idleFrame = first;
						}
					}
					else if(PORT_strncasecmp(name,"head_",5)==0)
					{
						if(headStart==-1)
						{
							headStart = first;
						...
						if(PORT_strcasecmp(name,"head_idle")==0)
						{
							m_headFrame = first;
						}
					}
		...
		if(legsStart>=torsoEnd)
		{
			...
			{
				if(PORT_strncasecmp(animNames[a].c_str(),"legs_",5)!=0)
					continue;
		...
		if(headStart>=bothEnd)
		{
			...
			for(size_t a = 0; a<m_animStartFrame.size(); ++a)
			{
				if(PORT_strncasecmp(animNames[a].c_str(),"head_",5)!=0)
					continue;
				if(m_animStartFrame[a]>=animOffset)
					m_animStartFrame[a] -= animOffset;
			}
...
void Md3Filter::readFile_setMeshes(MeshSectionE section, int32_t offsetMeshes, int32_t numMeshes, int32_t parentTag, int32_t animIndex)
{
	...
	for(int mesh = 0; mesh<numMeshes; mesh++)
	{
		//Mesh header
		int8_t meshMagic[4];
		m_src->readBytes(meshMagic,sizeof(meshMagic));
		...
		if(animIndex<0)
		{
			...
			//int32_t triang[meshTriangleCount][3]; //VLA
			//unsigned tri[meshTriangleCount]; //VLA
			...
			std::string skin = m_modelPath+shaderFileName;
		...
		if(PORT_strcasecmp(ext,".skin")==0)
		{
			...
			*/
			...
			while(src->readLine(linebuf,sizeof(linebuf)))
			{
				char *line = linebuf;
				chomp(linebuf);
				if(PORT_strncasecmp(line,meshName,meshNameLength)==0)
				{
					line += meshNameLength;
					if(line[0]==','&&line[1]!=0)
					{
...
bool Md3Filter::animInSection(std::string animName, MeshSectionE section)
{
	...
	if(PORT_strncasecmp(animName.c_str(),"torso_",6)==0)
	{
		if(section==MS_Upper)
			return true;
		else
	...
	if(PORT_strncasecmp(animName.c_str(),"legs_",5)==0)
	{
		if(section==MS_Lower)
			return true;
		else
	...
	if(PORT_strncasecmp(animName.c_str(),"head_",5)==0)
	{
		if(section==MS_Head)
			return true;
		else
	...
	if(PORT_strncasecmp(animName.c_str(),"both_",5)==0)
	{
	...
	if(PORT_strncasecmp(animName.c_str(),"all_",4)==0)
	{
...
bool Md3Filter::tagInSection(std::string tagName, MeshSectionE section)
{
	...
	if(PORT_strcasecmp(tagName.c_str(),"tag_torso")==0)
	{
		if(section!=MS_Head)
		{
			return true;
	...
	else if(PORT_strcasecmp(tagName.c_str(),"tag_head")==0)
	{
		if(section!=MS_Lower)
		{
			return true;
	...
	else if(PORT_strcasecmp(tagName.c_str(),"tag_weapon")==0
		// ZTM: FIXME?: Game specific
		// Support Team Arena tag point
	  ||PORT_strcasecmp(tagName.c_str(),"tag_flag")==0
		// Support Turtle Arena tag points
	  ||PORT_strcasecmp(tagName.c_str(),"tag_hand_primary")==0
	  ||PORT_strcasecmp(tagName.c_str(),"tag_hand_secondary")==0
	  ||PORT_strcasecmp(tagName.c_str(),"tag_wp_away_primary")==0
	  ||PORT_strcasecmp(tagName.c_str(),"tag_wp_away_secondary")==0
	  )
	{
		if(section==MS_Upper)
		{
...
bool Md3Filter::tagIsSectionRoot(std::string tagName, MeshSectionE section)
{
	switch (section)
	{
		...
		case MS_Head:
			if(PORT_strcasecmp(tagName.c_str(),"tag_head")==0)
			{
				return true;
			}
			break;
...
Model::ModelErrorE Md3Filter::writeFile(Model *model, const char *const filename, Options&)
{
	...
	{
		model->setFilterSpecificError(TRANSLATE("LowLevel","MD3 export requires all faces to be grouped."));
	...
	if(saveAsPlayer)
	{
		if(	 PORT_strncasecmp(modelBaseName.c_str(),"lower.",6)==0 
				...
				||PORT_strncasecmp(modelBaseName.c_str(),"head.", 5)==0)
		{
			...
			{
				char answer = msg_info_prompt(TRANSLATE("LowLevel","This looks like a player model.\nDo you want to save all sections?"),"Ync");
				if(answer=='Y')
				{
					model->addMetaData("MD3_composite","1");
				}
	...
	if(saveAsPlayer)
	{
		...
		model->operationComplete(TRANSLATE("LowLevel","Set meta data for MD3 export"));
...
Model::ModelErrorE Md3Filter::writeSectionFile(const char *filename,Md3Filter::MeshSectionE section,MeshList &meshes)
{
	...
	if(snprintf(pk3Name,sizeof(pk3Name),"%s%s",
				pk3Path.c_str(),modelBaseName.c_str())>=MAX_QPATH)
	{
		log_error("MD3_PATH+filename is too large\n");
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","MD3_PATH+filename is to long."));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	...
	if(numFrames>MD3_MAX_FRAMES)
	{
		...
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","Too many animation frames for MD3 export."));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	...
	if(numTags>MD3_MAX_TAGS)
	{
		...
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","Too many points for MD3 export."));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	if(numMeshes>MD3_MAX_SURFACES)
	{
		log_error("Number of groups(%d)is larger than %d\n.\n",numMeshes,MD3_MAX_SURFACES);
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","Too many groups for MD3 export."));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	...
	if(numTris>MD3_MAX_TRIANGLES)
	{
		...
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","Too many faces in a single group for MD3 export"));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	if(numVerts>MD3_MAX_VERTS)
	{
		log_error("Number of verticies(%d)is larger than %d\n.\n",numVerts,MD3_MAX_VERTS);
		m_model->setFilterSpecificError(TRANSLATE("LowLevel","Too many vertices in a single group for MD3 export"));
		return Model::ERROR_FILTER_SPECIFIC;
	}
	...
	m_dst->write(magic[3]);
	m_dst->write(version);
	m_dst->writeBytes(pk3Name,MAX_QPATH);
	m_dst->write(flags);
	...
	for(mlit = meshes.begin(); mlit!=meshes.end(); mlit++)
	{
		if((*mlit).group>=0&&groupInSection(m_model->getGroupName((*mlit).group),section))
		{
			...
			if(snprintf(mName,sizeof(mName),"%s",m_model->getGroupName((*mlit).group))>MAX_QPATH)
			{
				log_error("group name is too large\n");
				m_model->setFilterSpecificError(TRANSLATE("LowLevel","Group name is too large for MD3 export."));
				return Model::ERROR_FILTER_SPECIFIC;
			}
			...
			m_dst->writeBytes(mName,MAX_QPATH);
			m_dst->write(mFlags);
			...
			for(int32_t t = 0; t<mNumShaders; t++)
			{
				...
				if(strncmp(matFullName.c_str(),m_modelPath.c_str(),m_modelPath.size())==0)
				{
								...
								spk3Path.c_str(),matBaseName.c_str())>=MAX_QPATH)
					{
						...
						m_model->setFilterSpecificError(TRANSLATE("LowLevel","Texture filename is too long."));
						return Model::ERROR_FILTER_SPECIFIC;
					}
				}
...
bool Md3Filter::writeAnimations()
{
	...
	else
	{
		...
		for(unsigned int i = 0; i<m_model->getMetaDataCount(); i++)
		{
			...
			if(PORT_strncasecmp(keyword,"MD3_CFG_",8)==0)
			{
				if(!hadKeyword)
				{
					hadKeyword = true;
			...
			else if(PORT_strncasecmp(keyword,"MD3_sex",7)==0
			 ||PORT_strncasecmp(keyword,"MD3_footsteps",13)==0
			 ||PORT_strncasecmp(keyword,"MD3_headoffset",14)==0
			 ||PORT_strncasecmp(keyword,"MD3_fixedtorso",14)==0
			 ||PORT_strncasecmp(keyword,"MD3_fixedlegs",13)==0)
			{
				if(!hadKeyword)
				{
					hadKeyword = true;
			...
			else if(PORT_strncasecmp(keyword,"MD3_EliteLoop",13)==0)
			{
				eliteLoop = (atoi(value)>0);
			}
			else if(PORT_strncasecmp(keyword,"MD3_AnimKeyword",15)==0)
			{
...
Md3Filter::MeshAnimationTypeE Md3Filter::getAnimationType(const char *animName)
{
	MeshAnimationTypeE animType = MA_All;

	if(PORT_strncasecmp(animName,"both_",5)==0)
	{
		animType = MA_Both;
	}
	else if(PORT_strncasecmp(animName,"torso_",6)==0)
	{
		animType = MA_Torso;
	}
	else if(PORT_strncasecmp(animName,"legs_",5)==0)
	{
		animType = MA_Legs;
	}
	else if(PORT_strncasecmp(animName,"head_",5)==0)
	{
		animType = MA_Head;
	}
...
extern ModelFilter *md3filter(ModelFilter::PromptF f)
{
	auto o = new Md3Filter; o->setOptionsPrompt(f); return o;
}
