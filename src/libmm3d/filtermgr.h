...
public:
	...
	void registerFilter(ModelFilter *filter);
	...
	const char *getAllReadTypes();
	const char *getAllWriteTypes(bool exportModel = false);
...
protected:
	...
	std::vector<ModelFilter*> m_filters;

	std::string _read,_write,_export; //NEW
};
