		...
		const char *longOption, const char *defaultValue,bool takesArg)
{
	...
	m_optionList[id].reset(new Option(shortOption,longOption,defaultValue,takesArg));
}

void CommandLineManager::addCustomOption(int id,Option *opt)
{
	//m_optionList[id] = opt;
	m_optionList[id].reset(opt);
}
...
{
	//m_optionList[id] = new FunctionOption(shortOption,longOption,takesArg,optFunc);
	m_optionList[id].reset(new FunctionOption(shortOption,longOption,takesArg,optFunc));
}
