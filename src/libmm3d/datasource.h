...
//					otherwise.
//	*readAsciiz -- Reads until the next nullptr byte (or target buffer is 
...
class DataSource
{
	public:
		...
		bool readBytes(void *buf, size_t bufLen);
		...
		// pointer is not nullptr,its value is set to true if foundChar was
		// found and false otherwise.
		// Returns false if a read error occurred.
		bool readTo(char stopChar,char *buf, size_t bufLen,bool *foundChar);

		// Copy input data into buf until a nullptr byte is found (or bufLen is
		// exhausted). The nullptr itself is also copied. This is the same as
		// readTo()above with '\0' as the stopChar,except that buf is
		// gauranteed to be null-terminated.
		// Returns false if a read error occurred.
		bool readAsciiz(char *buf, size_t bufLen,bool *foundNull = nullptr);
		...
		// gauranteed to be null-terminated.
		// Returns false if a read error occurred.
		bool readLine(char *buf, size_t bufLen,bool *foundNewline = nullptr);
